...
- If someone clicks "Continue with Google" first and the email already exists as a password account, we show a toast: _"An account exists for this email. Sign in with your password first, then link Google from Settings."_ — matches your chosen "Ask for password first, then link" policy.
...
**Gating helper.** `useSubscription()` hook + `<RequirePro>` wrapper. When trial expires and no active sub, dashboard shows a soft paywall banner and disables _create session_, _import students_, and _generate portal link_. Read-only (reports, existing students) stays open so nothing gets lost.
